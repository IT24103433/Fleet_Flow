...
export const updateUserStatus = async (id, isActive, token) => {
  try {
    const headers = {
      'Content-Type': 'application/json',
      'Accept': 'application/json',
    ...
    const response = await fetch(`${IDENTITY_API_URL}/api/admin/users/${id}/status`, {
      method: 'PATCH',
      headers,
    ...
    if (!response.ok) {
      ...
      if (!message) {
        if (response.status === 401) {
          ...
          message = 'Access denied. Administrator privileges are required to change account status.';
        } else if (response.status === 404) {
          message = 'User account was not found.';
        } else if (response.status === 400) {
...
export const resetUserPassword = async (id, payload, token) => {
  try {
    const headers = {
      'Content-Type': 'application/json',
      'Accept': 'application/json',
    ...
    const response = await fetch(`${IDENTITY_API_URL}/api/admin/users/${id}/reset-password`, {
      method: 'POST',
      headers,
    ...
    if (!response.ok) {
      ...
      if (!message) {
        if (response.status === 401) {
          ...
          message = 'Access denied. Administrator privileges are required to reset passwords.';
        } else if (response.status === 404) {
          message = 'User account was not found.';
        } else if (response.status === 400) {
